...
<link rel="stylesheet"
	href="${pageContext.request.contextPath }/resources/css/edocSrchRead.css">
<link
...
<body>
		...
		<div>
			<article>
				...
				<div class="content">
					...
					<div class="tbRow">
						<iframe src="getHiddenAddr.do?attachId=${attachId }"
							frameborder="0" style="width: 88%; height: 100%"> </iframe>
					</div>
				</div>
			</article>
		</div>
	...
	<div id="flowModal" class="modal">
		...
		<div class="modal-content">
			...
			<div class="flowTbDiv">
				<form action="${pageContext.request.contextPath }/edoc/flowExe.do"
					...
					<table class="flowModalTb">
						...
						<c:forEach items="${edocFlowExList }" var="EdocFlow">
							<c:if test="${EdocFlow.flowCd eq 'F1'}">
								<tr>
									<td>결재자</td>
									<td>${EdocFlow.deptName }<br /> ${EdocFlow.jobName }
										${EdocFlow.empName }
									</td>
									<c:choose>
												...
												<c:when
													...
													<td>
														<div id="selectFlow" class="select-box">
															<div id="selectFlowSt" class="select-box__current"
																tabindex="1">
																<div class="select-box__value">
																	<input class="select-box__input" type="radio"
																		id="flowN" value="N" name="flowSt" />
																	<p id="flowNP" class="select-box__input-text">반려</p>
																</div>
																<div class="select-box__value">
																	<input class="select-box__input" type="radio"
																		id="flowY" value="Y" name="flowSt" checked="checked" />
																	<p id="flowYP" class="select-box__input-text">결재</p>
																</div>
													...
													<td><input type="hidden" name="flowEmpNo"
														value="${EdocFlow.flowEmpNo}" /> <input type="hidden"
														name="flowCd" value="${EdocFlow.flowCd}" /> <input
														type="hidden" name="edocId" value="${EdocFlow.edocId}" />
														<input type="hidden" name="edocTitleFromPage"
														value="${edocTitleFromPage }" />
														<button type="submit" class="whiteBtn submitBtn">제출</button></td>
												</c:when>
												<c:otherwise>
													<td></td>
													<td></td>
											...
											<c:choose>
												<c:when
													...
													<td><input type="hidden" name="flowEmpNo"
														...
														<input type="hidden" name="edocTitleFromPage"
														value="${edocTitleFromPage }" />
														<button type="submit">제출</button></td>
												</c:when>
												<c:otherwise>
													<td></td>
													<td></td>
											...
											<c:choose>
												<c:when
													...
													<td><textarea name="flowCmt" rows="3" cols="50"
															placeholder="협조의견을 입력하세요"></textarea></td>
													<td>
														<div id="selectFlow" class="select-box">
															<div id="selectFlowSt" class="select-box__current"
																tabindex="1">
